...
```{r, eval=TRUE}
nrow(cdc) # number of rows (observations)
dim(cdc)  # also give the number of observations 
```
    
    b) How many variables?  
    
```{r, eval=TRUE}
ncol(cdc) # number of columns (variables)
```    
...
nrow(cdc)
ncol(cdc)
dim(cdc)[2] # to see one observation in particular 
```

    
    c) For each of the following variables, identify its data type (categorical (nominal or ordinal) or numerical (continuos or discrete)).

genhlth: categorical (ordinal)

exerany: categorical (nominal)

height: numerical (continuos)
...
```{r,eval=TRUE}
barplot(smokebygen, xlim = c(0, 7000), names.arg = c('No', 'Yes'), legend.text = c('M', 'F'),
        beside = TRUE, horiz = TRUE, col = c('red', 'blue'), main = 'Barplot of smoke100 by Sex')
```
